    ...
    <div className="min-h-screen p-4 pt-16 sm:pt-16 lg:pt-10 sm:p-6 lg:p-10 max-w-screen-xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-8">
        {[
          ...
          { label: 'Revenue YTD', val: income?.ytd.total_income,          icon: TrendingUp,  cls: 'text-text-primary' },
          { label: 'NOI MTD',     val: income?.mtd.net_operating_income,  icon: BarChart3,  cls: 'text-accent' },
          { label: 'NOI YTD',     val: income?.ytd.net_operating_income,  icon: BarChart3,  cls: 'text-accent' },
          ...
          <div key={label} className="bg-surface border border-border/50 rounded-xl p-4 sm:p-5">
            ...
            {loading ? <Sk /> : <p className={`text-xl sm:text-2xl font-bold tabular-nums ${cls}`}>{fmt$(val ?? null)}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="bg-surface border border-border/50 rounded-xl p-4 sm:p-6 mb-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-4 sm:gap-8 text-xs font-semibold uppercase tracking-wider text-accent/70">
            <span>MTD</span><span className="w-24 sm:w-28 text-right">YTD</span>
          </div>
        </div>
        {loading ? (
          ...
          <div>
            ...
            <MetricRow label="Rental Income" ytd={income.ytd.rental_income}            mtd={income.mtd.rental_income} />
            <MetricRow label="Other Income"  ytd={income.ytd.other_income}             mtd={income.mtd.other_income} />
            <MetricRow label="Total Income"  ytd={income.ytd.total_income}             mtd={income.mtd.total_income} highlight="positive" />
            ...
            <MetricRow label="Operating Expenses" ytd={income.ytd.operating_expenses}  mtd={income.mtd.operating_expenses} />
            <MetricRow label="Total Expenses"     ytd={income.ytd.total_expenses}      mtd={income.mtd.total_expenses} highlight="negative" />
            <div className="mt-5 pt-4 border-t-2 border-border/60">
              <MetricRow label="Net Operating Income" ytd={income.ytd.net_operating_income} mtd={income.mtd.net_operating_income} highlight="positive" />
              {income.ytd.profit_margin != null && (
                <div className="flex items-center justify-between py-2">
                  <span className="text-sm text-text-secondary">Profit Margin</span>
                  <span className="text-sm font-semibold text-accent tabular-nums">{fmtPct(income.ytd.profit_margin)}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-surface border border-border/50 rounded-xl p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-5 gap-3">
          ...
          <form onSubmit={handleGLSearch} className="flex items-center gap-2">
            ...
            {glAccount && <button type="button" onClick={() => { setGlAccount(''); loadGL(''); }} className="text-xs text-text-muted hover:text-text-secondary">Clear</button>}
          </form>
        </div>
        {glEntries.length > 0 && (
            ...
            <tbody className="divide-y divide-border/25">
              {glLoading ? (
                ...
                <tr key={e.txn_id} className="hover:bg-surface-elevated/40 transition-colors">
                  <td className="px-3 py-2.5 text-xs text-text-secondary tabular-nums whitespace-nowrap">{fmtDate(e.post_date)}</td>
                  <td className="px-3 py-2.5 text-xs text-text-secondary max-w-[130px] truncate">{e.account ?? '—'}</td>
