  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-border/30 last:border-0">
      ...
      <div className="flex items-center gap-4 sm:gap-8">
        ...
        <span className={`text-sm font-semibold tabular-nums w-24 sm:w-28 text-right ${cls}`}>
          {fmt$(ytd)}
        </span>
      </div>
    </div>
...
export default function FinancialsContent() {
    ...
    async (account = '') => {
      ...
      try {
        const params = new URLSearchParams({
          _path: '/api/pages/financials/general-ledger',
          start_date: start,
          end_date: end,
        });
  ...
  const Sk = () => <div className="h-7 w-20 bg-surface-elevated animate-pulse rounded" />;
    ...
    <div className="min-h-screen p-4 pt-16 sm:pt-16 lg:pt-10 sm:p-6 lg:p-10 max-w-screen-xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-8">
        {[
          {
            label: 'Revenue MTD',
            val: income?.mtd.total_income,
            icon: DollarSign,
            cls: 'text-text-primary',
          },
          {
            label: 'Revenue YTD',
            val: income?.ytd.total_income,
            icon: TrendingUp,
            cls: 'text-text-primary',
          },
          {
      ...
      <div className="bg-surface border border-border/50 rounded-xl p-4 sm:p-6 mb-8">
        ...
        {loading ? (
          <div className="space-y-3">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="h-10 bg-surface-elevated animate-pulse rounded-lg" />
            ))}
          </div>
        ) : !income ? (
          <p className="text-sm text-text-muted text-center py-8">
            No data yet. Populates after the 6 AM pipeline run.
          </p>
        ) : (
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-accent/60 mb-2 pb-2 border-b border-border/40">
              Income
            </p>
            <MetricRow
              label="Rental Income"
              ytd={income.ytd.rental_income}
              mtd={income.mtd.rental_income}
            />
            <MetricRow
              label="Other Income"
              ytd={income.ytd.other_income}
              mtd={income.mtd.other_income}
            />
            <MetricRow
              label="Total Income"
              ytd={income.ytd.total_income}
              mtd={income.mtd.total_income}
              highlight="positive"
            />
            ...
            <MetricRow
              ...
              ytd={income.ytd.total_expenses}
              mtd={income.mtd.total_expenses}
              highlight="negative"
            />
      ...
      <div className="bg-surface border border-border/50 rounded-xl p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-5 gap-3">
          ...
          <form onSubmit={handleGLSearch} className="flex items-center gap-2">
            ...
            <button
              type="submit"
              disabled={glLoading}
              className="px-3 py-1.5 text-xs font-medium bg-accent/15 text-accent border border-accent/30 rounded-lg hover:bg-accent/25 disabled:opacity-40"
            >
              {glLoading ? '…' : 'Filter'}
            </button>
            {glAccount && (
        ...
        {glEntries.length > 0 && (
          <div className="flex flex-wrap gap-4 px-3 py-2.5 mb-4 rounded-lg bg-surface-elevated border border-border/40 text-xs text-text-secondary">
            <span>{glEntries.length} entries</span>
            <span className="flex items-center gap-1">
              <ArrowUpRight size={11} className="text-accent" />
              Credits:{' '}
              <span className="font-semibold text-accent tabular-nums ml-1">
                {fmt$(totalCredits)}
              </span>
            </span>
            <span className="flex items-center gap-1">
              <ArrowDownRight size={11} className="text-danger" />
              Debits:{' '}
              <span className="font-semibold text-danger tabular-nums ml-1">
                {fmt$(totalDebits)}
              </span>
            </span>
          </div>
          ...
          <table className="w-full text-sm min-w-[500px]">
            <thead>
              <tr className="border-b border-border/50">
                {['Date', 'Account', 'Description', 'Party', 'Credit', 'Debit'].map((h) => (
                  <th
                    key={h}
                    className="text-left px-3 py-2.5 text-xs font-semibold uppercase tracking-wider text-accent/80"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-border/25">
              {glLoading ? (
                  ...
                  <tr key={i}>
                    {[...Array(6)].map((_, j) => (
                      <td key={j} className="px-3 py-3">
                        <div className="h-3 bg-surface-elevated animate-pulse rounded w-16" />
                      </td>
                    ))}
                  </tr>
                ))
                ...
                glEntries.map((e) => (
                  <tr key={e.txn_id} className="hover:bg-surface-elevated/40 transition-colors">
                    <td className="px-3 py-2.5 text-xs text-text-secondary tabular-nums whitespace-nowrap">
                      {fmtDate(e.post_date)}
                    </td>
                    <td className="px-3 py-2.5 text-xs text-text-secondary max-w-[130px] truncate">
                      {e.account ?? '—'}
                    </td>
                    <td className="px-3 py-2.5 text-xs text-text-primary max-w-[160px] truncate">
                      {e.description ?? '—'}
                    </td>
                    <td className="px-3 py-2.5 text-xs text-text-secondary max-w-[110px] truncate">
                      {e.party_name ?? '—'}
                    </td>
                    <td className="px-3 py-2.5 text-xs font-medium text-accent tabular-nums">
                      {e.credit ? fmt$(e.credit) : '—'}
                    </td>
                    <td className="px-3 py-2.5 text-xs font-medium text-danger tabular-nums">
                      {e.debit ? fmt$(e.debit) : '—'}
                    </td>
                  </tr>
                ))
