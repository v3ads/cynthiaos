...
function BucketBar({
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <span className={`w-12 text-right tabular-nums font-medium flex-shrink-0 ${cls}`}>
        {label}
      </span>
      ...
      <span className="text-text-muted tabular-nums w-14 text-right flex-shrink-0">
        {fmt$(amount)}
      </span>
    </div>
  );
      ...
      <div className="flex items-start justify-between pb-6 border-b border-border/60 mb-8">
        <div>
          ...
          <p className="text-text-secondary text-sm mt-1.5">
            {records.length} tenants · accounts receivable aging
          </p>
        </div>
        <button
          onClick={() => load(bucket)}
                  ...
                  <tr key={r.tenant_id} className="hover:bg-surface-elevated/40 transition-colors">
                    <td className="px-4 py-3.5">
                      <p className="text-sm font-medium text-text-primary">
                        {fmtName(r.tenant_name)}
                      </p>
                      {r.tenant_status && (
                        <p className="text-xs text-text-muted capitalize">{r.tenant_status}</p>
                      )}
                    </td>
                    <td className="px-4 py-3.5 font-mono text-xs text-text-secondary">
                      {r.unit_id}
                    </td>
                    <td className="px-4 py-3.5 text-sm font-semibold text-danger tabular-nums">
                      {fmt$(r.total_amount)}
                    </td>
                    <td className="px-4 py-3.5 min-w-[180px]">
                      <div className="space-y-1">
                        <BucketBar
                          label="0–30d"
                          amount={r.amount_0_to_30}
                          total={r.total_amount}
                          cls="text-text-muted"
                        />
                        <BucketBar
                          label="31–60d"
                          amount={r.amount_30_to_60}
                          total={r.total_amount}
                          cls="text-warning"
                        />
                        <BucketBar
                          label="61–90d"
                          amount={r.amount_60_to_90}
                          total={r.total_amount}
                          cls="text-danger"
                        />
                        <BucketBar
                          label="90d+"
                          amount={r.amount_90_plus}
                          total={r.total_amount}
                          cls="text-danger"
                        />
                      </div>
