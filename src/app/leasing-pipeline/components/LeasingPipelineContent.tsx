...
function StatusPill({ status }: { status: string }) {
  return (
    <span
      className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full border ${statusColor(status)}`}
    >
      {status || '—'}
    </span>
  );
...
export default function LeasingPipelineContent() {
  ...
  const prospectStatuses = [
    'all',
    ...Array.from(new Set(prospects.map((p) => p.status).filter(Boolean))),
  ];
  const applicantStatuses = [
    'all',
    ...Array.from(new Set(applicants.map((a) => a.status).filter(Boolean))),
  ];
    ...
    <div className="min-h-screen p-4 pt-16 sm:pt-16 lg:pt-10 sm:p-6 lg:p-10 max-w-screen-xl mx-auto">
      <div className="flex items-start justify-between pb-6 border-b border-border/60 mb-8">
        <div>
          ...
          <p className="text-text-secondary text-sm mt-1.5">
            Active prospects and applicants from AppFolio
          </p>
        </div>
        <button
          onClick={load}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-8">
        {[
          ...
          {
            label: 'Converted',
            val: convertedApplicants.length,
            cls: 'text-accent',
            bg: 'bg-accent/15',
            icon: UserCheck,
          },
              ...
              <tr className="border-b border-border/50">
                {tab === 'prospects'
                  ? [
                      ...
                      <th
                        key={h}
                        className="text-left px-3 py-3 text-xs font-semibold uppercase tracking-wider text-accent/80"
                      >
                        {h}
                      </th>
                    ))
              ...
              {loading ? (
                ...
                filteredProspects.length === 0 ? (
                  ...
                  filteredProspects.map((p) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 text-xs text-text-secondary">
                        {p.bed_bath_preference ?? '—'}
                      </td>
                      <td className="px-3 py-3 text-xs font-medium text-text-primary tabular-nums">
                        {fmt$(p.max_rent)}
                      </td>
                      <td className="px-3 py-3 text-xs text-text-secondary tabular-nums">
                        {fmtDate(p.move_in_preference)}
                      </td>
                      <td className="px-3 py-3 text-xs text-text-secondary tabular-nums">
                        {fmtDate(p.last_activity_date)}
                      </td>
                    </tr>
                  ))
                )
                ...
                filteredApplicants.map((a) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      <StatusPill status={a.application_status ?? a.status} />
                    </td>
