...
export default function UpcomingRenewalsContent() {
  ...
  const handleSave = async (unitId: string) => {
    ...
    if (ok) {
      ...
      setEditStates((prev) => ({
        ...prev,
        [unitId]: { ...prev[unitId], dirty: false, saving: false },
      }));
        ...
        prev.map((r) =>
          r.unit_id === unitId
            ? {
                ...
                proposed_rent: payload.proposed_rent ?? null,
  ...
  const formatDate = (dateStr: string | null) =>
    dateStr
      ? new Date(dateStr.length === 10 ? dateStr + 'T12:00:00' : dateStr).toLocaleDateString(
          'en-US',
  ...
  const statusCounts = {
    ...
    pending: records.filter(
      (r) => (editStates[r.unit_id]?.renewal_status ?? r.renewal_status) === 'pending'
    ).length,
    in_progress: records.filter(
      (r) => (editStates[r.unit_id]?.renewal_status ?? r.renewal_status) === 'in_progress'
    ).length,
    signed: records.filter(
      (r) => (editStates[r.unit_id]?.renewal_status ?? r.renewal_status) === 'signed'
    ).length,
    declined: records.filter(
      (r) => (editStates[r.unit_id]?.renewal_status ?? r.renewal_status) === 'declined'
    ).length,
  };
      ...
      {!loading && statusCounts.declined > 0 && (
        <div className="flex items-center gap-3 bg-danger/8 border border-danger/25 rounded-xl px-4 py-3 mb-6">
          ...
          <p className="text-sm text-danger font-medium">
            {statusCounts.declined} tenant{statusCounts.declined > 1 ? 's' : ''} declined renewal —
            begin re-leasing to prevent vacancy.
          </p>
        </div>
      )}
      ...
      <div className="bg-surface border border-border rounded-xl">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 border-b border-border/50">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {(['ALL', 'pending', 'in_progress', 'signed', 'declined'] as RenewalStatusFilter[]).map(
              (status) => (
                <button
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border/30">
                {paginated.map((renewal) => {
                  ...
                  const urgencyColor =
                    renewal.days_until_expiration <= 30
                      ? 'text-danger'
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleFieldChange(renewal.unit_id, 'proposed_rent', e.target.value)
                            }
                            placeholder="—"
                      ...
                      <td className="px-4 py-3">
                        <div className="relative">
                          <select
                            ...
                            onChange={(e) =>
                              handleFieldChange(renewal.unit_id, 'renewal_status', e.target.value)
                            }
                      ...
                      <td className="px-4 py-3">
                        <input
                          ...
                          value={es?.notes ?? ''}
                          onChange={(e) =>
                            handleFieldChange(renewal.unit_id, 'notes', e.target.value)
                          }
