        ...
        <div>
          ...
          <div className="space-y-1.5">
            {rows.map((r) => (
              <div
                key={r.label}
                className="flex justify-between py-0.5 border-b border-border/20 last:border-0"
              >
...
export default function MaintenanceContent() {
  ...
  const completedMonth = allOrders.filter(
    (r) => COMP_STATUSES.has(r.status) && isCurrentMonth(r.completed_on)
  ).length;
      ...
      <div className="bg-surface border border-border/40 rounded-xl overflow-hidden">
        ...
        <div className="hidden md:grid grid-cols-[3rem_6rem_7rem_1fr_7rem_6rem_6rem_6rem] border-b border-border/40 px-4">
          {['', 'Unit', 'Tenant', 'Issue', 'Status', 'Created', 'Completed', ''].map((h, i) => (
            <div
              key={i}
              className="py-2.5 text-xs font-semibold uppercase tracking-wider text-accent/80"
            >
              {h}
            </div>
          ))}
        ...
        {loading || !hasLoaded ? (
          <div className="divide-y divide-border/30">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="hidden md:grid grid-cols-[3rem_6rem_7rem_1fr_7rem_6rem_6rem_6rem] px-4 py-3.5 animate-pulse"
              >
