      ...
      <div className="flex items-start justify-between pb-6 border-b border-border/60 mb-8">
        <div>
          ...
          <p className="text-text-secondary text-sm mt-1.5">
            Make-ready history and turnaround performance
          </p>
        </div>
        <button
          onClick={load}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4 mb-8">
        {[
          ...
          },
          {
            ...
            val: avgTurn !== null ? `${avgTurn}d` : '—',
            icon: Clock,
            cls: avgTurn && avgTurn > 21 ? 'text-warning' : 'text-accent',
            bg: avgTurn && avgTurn > 21 ? 'bg-warning/15' : 'bg-accent/15',
          },
          {
            label: 'Over 30 Days',
            val: longTurns,
            icon: Clock,
            cls: longTurns > 0 ? 'text-danger' : 'text-text-primary',
            bg: longTurns > 0 ? 'bg-danger/15' : 'bg-surface-elevated',
          },
          ...
          <div key={label} className="bg-surface border border-border/50 rounded-xl p-4 sm:p-5">
            ...
            {loading ? (
              <div className="h-7 w-12 bg-surface-elevated animate-pulse rounded" />
            ) : (
            ...
            <thead>
              <tr className="border-b border-border/50">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-accent/80"
                  >
                    {h}
                  </th>
                ))}
                  ...
                  <tr key={i}>
                    {[...Array(7)].map((_, j) => (
                      <td key={j} className="px-4 py-3.5">
                        <div className="h-3.5 bg-surface-elevated animate-pulse rounded w-16" />
                      </td>
                    ))}
                  </tr>
                ))
                ...
                turns.map((t, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-surface-elevated/40 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-xs text-text-secondary tabular-nums whitespace-nowrap">
                        {fmtDate(t.move_out_date)}
                      </td>
                      <td className="px-4 py-3.5 text-xs text-text-secondary tabular-nums whitespace-nowrap">
                        {fmtDate(t.expected_move_in_date)}
                      </td>
                      <td className="px-4 py-3.5 text-xs text-text-secondary tabular-nums">
                        {t.target_days ? `${t.target_days}d` : '—'}
                      </td>
                      <td className="px-4 py-3.5">
                        {actual !== null ? (
                          <span className={`text-sm tabular-nums ${actualCls}`}>{actual}d</span>
                        ) : (
                          <span className="text-xs text-text-muted">—</span>
                        )}
                      </td>
                      <td className="px-4 py-3.5 text-sm font-medium text-text-primary tabular-nums">
                        {fmt$(t.total_billed)}
                      </td>
                      <td className="px-4 py-3.5">
                        {t.status === 'scheduled' ? (
