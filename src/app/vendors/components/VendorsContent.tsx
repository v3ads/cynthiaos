...
export default function VendorsContent() {
  ...
  const filtered = vendors.filter((v) => {
    ...
    if (!q) return true;
    return [v.company_name ?? '', v.vendor_type ?? '', v.vendor_trades ?? ''].some((s) =>
  ...
  const VendorCard = ({ v }: { v: Vendor }) => {
    ...
    return (
      <div
        className={`bg-surface border rounded-xl p-4 sm:p-5 ${v.do_not_use ? 'border-danger/30 opacity-60' : 'border-border/50 hover:border-border transition-colors'}`}
      >
        ...
        {v.vendor_type && (
          <p className="text-xs text-accent/80 mb-3 font-medium">
            {v.vendor_type}
            {v.vendor_trades ? ` · ${v.vendor_trades}` : ''}
          </p>
        )}
        <div className="space-y-1.5 border-t border-border/30 pt-3">
          {v.phone_numbers && (
            <a
              href={`tel:${v.phone_numbers}`}
      ...
      {loading || !hasLoaded ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => (
            <div
              key={i}
              className="bg-surface border border-border/50 rounded-xl p-5 h-36 animate-pulse"
            />
          ))}
        </div>
        ...
        <>
          ...
          {blocked.length > 0 && (
            <>
              <p className="text-xs font-semibold uppercase tracking-wider text-danger/70 mb-3">
                Do Not Use
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {blocked.map((v) => (
