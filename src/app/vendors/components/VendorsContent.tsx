...
function cleanName(name: string | null): string | null {
  if (!name || name.includes('${') || !name.trim()) return null;
...
export default function VendorsContent() {
  ...
  const allTypes = Array.from(new Set(vendors.map(v => v.vendor_type ?? '').filter(Boolean))).sort();

  const q = search.trim().toLowerCase();
  const filtered = vendors.filter(v => {
    if (tradeFilter !== 'all' && v.vendor_type !== tradeFilter) return false;
    if (!q) return true;
    return [(v.company_name ?? ''), (v.vendor_type ?? ''), (v.vendor_trades ?? '')].some(s => s.toLowerCase().includes(q));
  });
  ...
  const VendorCard = ({ v }: { v: Vendor }) => {
    const contactName = cleanName(v.full_name);
    return (
      <div className={`bg-surface border rounded-xl p-4 sm:p-5 ${v.do_not_use ? 'border-danger/30 opacity-60' : 'border-border/50 hover:border-border transition-colors'}`}>
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          {v.do_not_use && (
            <span className="flex items-center gap-1 text-xs font-medium px-1.5 py-0.5 rounded-full bg-danger/10 text-danger border border-danger/25 flex-shrink-0">
              <AlertCircle size={9} />DNU
            </span>
          )}
        </div>
        {v.vendor_type && <p className="text-xs text-accent/80 mb-3 font-medium">{v.vendor_type}{v.vendor_trades ? ` · ${v.vendor_trades}` : ''}</p>}
        <div className="space-y-1.5 border-t border-border/30 pt-3">
          {v.phone_numbers && (
            <a href={`tel:${v.phone_numbers}`} className="flex items-center gap-2 text-xs text-text-secondary hover:text-text-primary transition-colors">
              <Phone size={11} className="text-text-muted flex-shrink-0" />{v.phone_numbers}
            </a>
          )}
          {v.email && (
  ...
  return (
    <div className="min-h-screen p-4 pt-16 sm:pt-16 lg:pt-10 sm:p-6 lg:p-10 max-w-screen-xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20"><p className="text-sm font-medium text-text-primary">No vendors found</p></div>
      ) : (
        <>
          {active.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-8">
              {active.map(v => <VendorCard key={v.vendor_id} v={v} />)}
            </div>
          )}
          {blocked.length > 0 && (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {blocked.map(v => <VendorCard key={v.vendor_id} v={v} />)}
              </div>
            </>
          )}
        </>
