      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border/50">
            <th className="text-left px-4 py-3 text-xs font-semibold text-accent/80 uppercase tracking-wider">
              Tenant
            </th>
            <th className="text-left px-4 py-3 text-xs font-semibold text-accent/80 uppercase tracking-wider">
              Unit
            ...
            {showActions && !compact && (
              <th className="text-right px-4 py-3 text-xs font-semibold text-accent/80 uppercase tracking-wider">
                Actions
              </th>
            )}
          </tr>
        ...
        <tbody className="divide-y divide-border/30">
          {leases.map((lease) => {
            ...
            const badgeVariant =
              urgency === 'HIGH' ? 'danger' : urgency === 'MEDIUM' ? 'warning' : 'success';
            ...
            return (
              <tr
                ...
                className={`hover:bg-surface-elevated/50 transition-colors group ${
                  isHighUrgent
                    ? 'bg-danger/8 border-l-2 border-l-danger'
                    : isFamily
              ...
              >
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2.5">
                    <div
                      className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-semibold ${isHighUrgent ? 'bg-danger/20 text-danger' : 'bg-surface-elevated text-text-secondary'}`}
                    >
                      {(lease.tenant_name || '?')
                        .split(' ')
                        .map((n) => n[0] ?? '')
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        <p
                          className={`font-medium ${isHighUrgent ? 'text-text-primary font-semibold' : 'text-text-primary'}`}
                        >
                          {lease.tenant_name}
                        </p>
                        {isContacted && (
                      ...
                      {!compact && (
                        <p className="text-xs text-text-secondary">{lease.contact_email}</p>
                      )}
                    </div>
                  </div>
                </td>
                ...
                <td className="px-4 py-3">
                  <span className={`tabular-nums font-semibold ${config.textClass}`}>
                    {lease.days_until_expiration}d
                  </span>
                </td>
                <td className="px-4 py-3">
                ...
                {!compact && (
                  <td className="px-4 py-3 text-text-muted text-xs">{lease.lease_type}</td>
                )}
                {showActions && !compact && (
                  <td className="px-4 py-3">
