...
const VERDICTS = ["NAMING_MISMATCH", "SOURCE_NARROW", "BRONZE_COUNT_UNRESOLVED", "PROMOTION_STALLED", "CLEAN"];
...
const FETCH_TO_REGISTRY_ALIASES = {
...
function canonicalRegistryType(fetchType) {
    return FETCH_TO_REGISTRY_ALIASES[fetchType] ?? fetchType;
}
async function loadRegisteredStrategyTypes(registryPath = REGISTRY_PATH) {
    let source;
    try {
...
function isInScope(fetchType, registeredStrategyTypes) {
    if (MAINTENANCE_REPORT_TYPES.has(fetchType))
        return false;
...
function resolvePollUrl(location) {
    if (/^https?:\/\//i.test(location))
        return location;
    return new URL(location, APPFOLIO_BASE_URL).toString();
}
async function fetchAppFolioCount(entry) {
    const first = await fetchJson(entry.url);
    if (first.status === 200)
        return extractRows(first.payload).length;
    if (first.status !== 202) {
    ...
    if (!pollUrl && first.payload && typeof first.payload === "object") {
        const obj = first.payload;
        const candidate = obj.location ?? obj.url ?? obj.poll_url ?? obj.status_url;
        if (typeof candidate === "string")
            pollUrl = candidate;
    }
    if (!pollUrl)
        throw new Error(`AppFolio ${entry.id} returned 202 without a poll URL`);
    for (let attempt = 1; attempt <= POLL_MAX_ATTEMPTS; attempt += 1) {
        await sleep(POLL_INTERVAL_MS);
        const polled = await fetchJson(resolvePollUrl(pollUrl));
        if (polled.status === 200)
            return extractRows(polled.payload).length;
        if (polled.status !== 202)
            throw new Error(`AppFolio ${entry.id} poll returned HTTP ${polled.status}`);
    }
    throw new Error(`AppFolio ${entry.id} polling exceeded ${POLL_MAX_ATTEMPTS} attempts`);
}
async function ensurePipelineHealthTable(sql) {
    await sql `
...
async function queryBronze(sql, reportType) {
...
async function querySilver(sql, reportType) {
    const rows = await sql `
    WITH latest AS (
...
async function queryGoldOne(sql, mapping) {
    if (!(await tableExists(sql, mapping.table)))
        return { count: null, latestDate: null };
    ...
    const rows = await sql `SELECT COUNT(*)::integer AS row_count FROM ${sql(mapping.table)}`;
    return { count: rows[0]?.row_count ?? null, latestDate: null };
}
async function queryGold(sql, registryType) {
    ...
    const counts = snapshots.map((s) => s.count).filter((v) => typeof v === "number");
    const dates = snapshots.map((s) => s.latestDate).filter((v) => typeof v === "string" && v.length > 0).sort();
...
function daysBetween(older, newer) {
    ...
    if (Number.isNaN(olderDate.getTime()) || Number.isNaN(newerDate.getTime()))
        return null;
    return Math.floor((newerDate.getTime() - olderDate.getTime()) / 86400000);
}
function materiallyGreater(source, bronze) {
    if (source === null || bronze === null)
        return false;
...
function deriveVerdict(args) {
    ...
    if (countStalled || dateStalled) {
        if (countStalled)
            notes.push("Latest Bronze payload row count exceeds mapped Gold row count.");
        if (dateStalled)
            notes.push(`Gold latest date lags Bronze latest date by ${lagDays} days, above tolerance ${args.toleranceDays}.`);
...
async function insertHealthRows(sql, rows, runAt) {
...
function printTable(rows) {
    ...
    for (const row of rows) {
        if (row.notes.length > 0)
            console.log(`${row.report_type} notes: ${row.notes.join(" | ")}`);
    }
}
async function main() {
    ...
    try {
        ...
        const prioritized = catalogue.sort((a, b) => {
            const rank = (id) => id === "aged_receivables_detail" ? 0 : id === "tenant_directory" ? 1 : 2;
        ...
        for (const entry of prioritized) {
            ...
            if (!isDeduplicated && bronze.latestDate) {
                ...
                if (firstMapping?.dateColumn) {
                    ...
                    if (dateColExists) {
                        const dateScoped = await pooledSql `
